 ...
 * Maintains an array of ManagementRecords (MRs), one per potential visiting aircraft. 
 * Some MRs hold information about aircraft currently being managed by SAAMS, and some may have the status "Free".
 * The index of each ManagementRecord in the array is its "management code" ("mCode"), and the mCode of any particular 
 * visiting aircraft's ManagementRecord must remain fixed once it is allocated.
...
public class AircraftManagementDatabase extends Observable 
{
	...
	/**
	 * The size of the array MRs holding ManagementRecords. In this simple system 10 will do!
	 */
  ...
  /**
	 ...
	 * Parameter newStatus is the requested new status. No effect is expected if the current status 
	 * is not a valid preceding status. This operation is appropriate when the status change does not 
	 * need any additional information to be noted. It is present instead of a large collection of 
	 * public operations for requesting specific status changes.
	 */
	  public void setStatus(int mCode, int newStatus)
	  {
		  if(newStatus >= 0 && newStatus <= 18)
		  {
			  if(mCode < maxMRs && mCode >= 0)
			  {
				  if(newStatus != 6)
				  {
	...
	/**
	 * Returns an array of mCodes: just mCodes of management records that have the status
	...
	/**
	 * A GOC has allocated the given gate to the aircraft with the given mCode supplied as a parameter 
	 * for unloading passengers. The message is forwarded to the given MR for status update.
	 */
	...
	/**
	 *  The Maintenance Supervisor has reported faults with the given description in the aircraft with 
	 *  the given mCode. The message is forwarded to the given MR for status update.
	 *  
	 */
	...
	/**
	 *  The given passenger is boarding the aircraft with the given mCode. Forward the message to the 
	 *  given MR for recording in the passenger list.
	 *  */
